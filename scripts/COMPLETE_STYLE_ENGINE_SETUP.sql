-- =====================================================
-- COMPLETE STYLE ENGINE SETUP
-- This script ensures tables AND functions exist
-- Run this to fix "relation does not exist" errors
-- =====================================================

-- 1. User Events Table (Check existence first, but using CREATE IF NOT EXISTS)
...
-- Fix permissions for user_events
...
-- Fix permissions for user_style_profile
...
DROP POLICY IF EXISTS "Users can insert own profile" ON public.user_style_profile;
CREATE POLICY "Users can insert own profile" ON public.user_style_profile FOR INSERT WITH CHECK (auth.uid() = user_id);

-- 3. Re-create Functions (Drop first to be safe)
DROP FUNCTION IF EXISTS public.recalculate_style_profile(UUID);
DROP FUNCTION IF EXISTS public.get_style_twins(UUID);

-- Function to Recalculate Style Profile
...
BEGIN
    ...
    expanded_tags AS (
        SELECT jsonb_array_elements_text(payload->'tags') as tag, score FROM scores WHERE payload ? 'tags'
    ),
    expanded_colors AS (
        SELECT jsonb_array_elements_text(payload->'colors') as color, score FROM scores WHERE payload ? 'colors'
    ),
    expanded_categories AS (
        SELECT payload->>'category' as category, score FROM scores WHERE payload->>'category' IS NOT NULL
    )
    
    -- Calculate top tags
    SELECT array_agg(tag ORDER BY total_score DESC) INTO final_tags
    FROM (SELECT tag, SUM(score) as total_score FROM expanded_tags GROUP BY tag HAVING SUM(score) > 0 LIMIT 5) t;
    
    -- Calculate top colors
    SELECT array_agg(color ORDER BY total_score DESC) INTO final_colors
    FROM (SELECT color, SUM(score) as total_score FROM expanded_colors GROUP BY color HAVING SUM(score) > 0 LIMIT 5) c;
    
    -- Calculate top categories
    SELECT array_agg(category ORDER BY total_score DESC) INTO final_categories
    FROM (SELECT category, SUM(score) as total_score FROM expanded_categories GROUP BY category HAVING SUM(score) > 0 LIMIT 3) cat;
    ...
    RETURN jsonb_build_object('tags', final_tags, 'colors', final_colors, 'categories', final_categories);
END;
$$ LANGUAGE plpgsql SECURITY DEFINER;

-- Function to Find Style Twins
...
BEGIN
    ...
    IF my_tags IS NULL THEN RETURN; END IF;
