...
export function convertSizesToSizeChart(sizes) {
  ...
  return sizes.map(size => {
    ...
    // Database stores in cm, but fitLogic's toCm() can handle numbers (treats as cm) or strings
    // We'll pass as numbers to ensure fitLogic treats them as cm
    if (size.chest_width != null) measurements.chest = Number(size.chest_width);
    if (size.waist_width != null) measurements.waist = Number(size.waist_width);
    if (size.hip_width != null) measurements.hips = Number(size.hip_width);
    if (size.garment_length != null) measurements.length = Number(size.garment_length);
    
    // Upper body measurements
    if (size.shoulder_width != null) measurements.shoulder = Number(size.shoulder_width);
    if (size.sleeve_length != null) measurements.sleeve = Number(size.sleeve_length);
    
    // Lower body measurements
    if (size.inseam != null) measurements.inseam = Number(size.inseam);
    if (size.rise != null) measurements.rise = Number(size.rise);
    if (size.thigh_width != null) measurements.thigh = Number(size.thigh_width);
    if (size.leg_opening != null) measurements.leg_opening = Number(size.leg_opening);
