...
function colorFamily(color) {
  ...
  const warm = ["brown","camel","rust","terracotta","mustard","olive","khaki","orange","warm red","maroon"];
  const cool = ["navy","blue","cobalt","teal","emerald","cool pink","lavender","purple","icy blue"];
  ...
  // fallback: attempt keyword match
  if (c.includes("navy") || c.includes("blue")) return "cool";
  if (c.includes("beige") || c.includes("cream")) return "neutral";
  if (c.includes("brown") || c.includes("rust") || c.includes("olive")) return "warm";
...
function colorSuitability(user, product) {
  ...
  // Only show insufficient data if BOTH are missing
  if ((!undertone && !season) && !primaryColor) {
    return {
  ...
  // If only user profile is missing, still try to analyze with product color
  if (!undertone && !season) {
    // Can't give accurate advice without user profile, but can still show something
    return {
      status: "INSUFFICIENT_DATA",
      verdict: null,
      reasons: ["Set your Color Profile (undertone/season) to get accurate color advice."],
      alternatives: [],
    };
  }
  
  // If only product color is missing, can't analyze
  if (!primaryColor) {
    return {
      status: "INSUFFICIENT_DATA",
      verdict: null,
      reasons: ["We couldn't detect the product's main color. Add product color or use a link with color info."],
      alternatives: [],
    };
  }
