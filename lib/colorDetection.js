 ...
 * Uses server-side API with sharp for accurate color detection
 * Works with all image formats including HEIC
 * Falls back to client-side method for data URIs
 */
...
export async function detectDominantColor(imageUri) {
  ...
  try {
    // Try server-side API first (uses sharp for accurate color detection)
    // Fall back to client-side if API fails
    console.log('🎨 [CLIENT COLOR] Attempting server-side color detection...');
    ...
    if (imageUri.startsWith('data:')) {
      ...
      console.log('🎨 [CLIENT COLOR] Reading local file...');
      try {
        ...
        console.log('🎨 [CLIENT COLOR] Converted local file to base64');
      } catch (fileError) {
        console.error('🎨 [CLIENT COLOR] Error reading local file:', fileError);
        throw fileError;
      }
    } else {
    ...
    // Try server-side API
    const API_BASE = process.env.EXPO_PUBLIC_API_BASE || 'https://mvp-styli.vercel.app';
    const apiUrl = `${API_BASE}/api/detect-color`;
    
    try {
      console.log('🎨 [CLIENT COLOR] Calling server API:', apiUrl);
      ...
      if (response.ok) {
        ...
        console.log('🎨 [CLIENT COLOR] Server API result:', JSON.stringify(result, null, 2));
        
        // Cache the result if it's not grey/unknown
        if (cacheKey && result.name && result.name !== 'grey' && result.name !== 'gray' && result.name !== 'unknown') {
          colorCache[cacheKey] = result;
          console.log('🎨 [CLIENT COLOR] Cached color for:', cacheKey, '->', result.name);
        }
        
        return result;
      } else {
        console.warn('🎨 [CLIENT COLOR] Server API returned:', response.status);
      }
    } catch (apiError) {
      console.warn('🎨 [CLIENT COLOR] Server API failed, using client-side fallback:', apiError.message);
    }
    
    // Fallback to client-side method
    console.log('🎨 [CLIENT COLOR] Using client-side color detection fallback...');
    const dataUri = imageBase64 || (imageUrl ? await (async () => {
    ...
    return await detectColorFromDataUri(dataUri, cacheKey);
  } catch (error) {
    console.error('🎨 [CLIENT COLOR] Error detecting color:', error);
...
/**
 * Fallback method for data URIs (when react-native-image-colors can't be used)
 */
async function detectColorFromDataUri(dataUri, cacheKey) {
  try {
    // Extract color from base64 image data (pass cacheKey for caching control)
    const dominantColor = extractColorFromBase64(dataUri, cacheKey);
    console.log('🎨 [CLIENT COLOR] Fallback method detected color:', dominantColor);
    return dominantColor;
  } catch (error) {
    console.error('🎨 [CLIENT COLOR] Fallback method error:', error);
    return {
      color: '#000000',
      name: 'unknown',
      confidence: 0,
