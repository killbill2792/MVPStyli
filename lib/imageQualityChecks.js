...
export async function checkBrightness(imageUri) {
  try {
    // Read image as base64 to analyze pixels
    const base64 = await FileSystem.readAsStringAsync(imageUri, {
      encoding: FileSystem.EncodingType.Base64,
    });
    
    // Convert base64 to buffer (simplified - in practice you'd decode properly)
    // For now, use a simple heuristic based on file size and image dimensions
    const imageInfo = await Image.getSize(imageUri);
