...
// Universal URL parsing for ANY clothing website
const parseUrlFallback = async (url) => {
  try {
    console.log('Universal URL parsing for:', url);
    const domain = new URL(url).hostname.toLowerCase();
    console.log('Domain:', domain);
    
    // Extract product name from URL with universal intelligence
    const urlParts = url.split('/');
    let productName = "Detected Product";
    let brand = "Online Store";
    
    // Universal product name extraction - works for ANY website
    ...
    if (productPart) {
      // Universal product name cleaning
      productName = productPart
        ...
        .trim()
        .split(' ')
        .filter(word => word.length > 0 && word.length < 20) // Remove empty strings and very long words
        .map(word => word.charAt(0).toUpperCase() + word.slice(1).toLowerCase())
        .join(' ');
      
      console.log('Extracted product name:', productName);
    } else {
      // Fallback: extract from the last meaningful URL segment
    ...
    if (mainDomain && mainDomain !== 'www') {
      ...
      if (brand.toLowerCase().includes('prettylittlething')) brand = 'Pretty Little Thing';
      else if (brand.toLowerCase().includes('urbanoutfitters')) brand = 'Urban Outfitters';
      else if (brand.toLowerCase().includes('forever21')) brand = 'Forever 21';
      else if (brand.toLowerCase().includes('abercrombie')) brand = 'Abercrombie & Fitch';
      else if (brand.toLowerCase().includes('massimodutti')) brand = 'Massimo Dutti';
      else if (brand.toLowerCase().includes('pullandbear')) brand = 'Pull & Bear';
    }
    
    console.log('Detected brand:', brand);
    
    // Universal category detection from URL and product name
    ...
    console.log('Universal parsed result:', result);
    return result;
  } catch (error) {
    console.error('Error in universal URL parsing:', error);
    // Always return a valid product, even if parsing fails
    return {
      id: `detected-${Date.now()}`,
      name: "Detected Product",
