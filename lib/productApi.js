...
export const analyzeProductUrl = async (url) => {
  try {
    ...
    console.log('Calling scraping API...');
    ...
    if (response.ok) {
      ...
      if (productData && productData.name) {
        ...
        console.log('Returning scraped product:', result);
        return result;
      }
    } else {
      const errorText = await response.text();
      console.log('Scraping API error:', errorText);
    }
    
    // Fallback to URL parsing if scraping fails
    console.log('Scraping failed, using URL parsing fallback');
    const fallbackResult = await parseUrlFallback(url);
...
const parseUrlFallback = async (url) => {
  try {
    console.log('Parsing URL fallback for:', url);
