...
function recommendSizeAndFit(user, product, opts = {}) {
  ...
  const u = {
    heightIn: toInches(user?.heightIn ?? user?.heightCm ? (user.heightCm / 2.54) : null),
    weightKg: safeNum(user?.weightKg),
    chestIn: toInches(user?.chestIn ?? user?.chestCm ? (user.chestCm / 2.54) : null),
    bustIn: toInches(user?.bustIn ?? user?.bustCm ? (user.bustCm / 2.54) : null) ?? toInches(user?.chestIn ?? user?.chestCm ? (user.chestCm / 2.54) : null), // fallback
    waistIn: toInches(user?.waistIn ?? user?.waistCm ? (user.waistCm / 2.54) : null),
    hipsIn: toInches(user?.hipsIn ?? user?.hipsCm ? (user.hipsCm / 2.54) : null),
    shoulderIn: toInches(user?.shoulderIn ?? user?.shoulderCm ? (user.shoulderCm / 2.54) : null),
    inseamIn: toInches(user?.inseamIn ?? user?.inseamCm ? (user.inseamCm / 2.54) : null),
  };
