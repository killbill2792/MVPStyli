...
// Import all the helper functions from the index.ts file
...
async function scrapeHMProduct(url: string) {
  try {
    ...
    const descMatch = html.match(/<meta property="og:description" content="([^"]+)"/i);
    const description = descMatch ? descMatch[1] : '';
    return { name, price, image, description, brand: 'H&M', category: detectCategoryFromUrl(url, name) };
  } catch (error) {
...
async function scrapeUniqloProduct(url: string) {
  try {
    ...
    const imageMatch = html.match(/<meta property="og:image" content="([^"]+)"/i);
    const image = imageMatch ? imageMatch[1] : '';
    return { name, price, image, brand: 'Uniqlo', category: detectCategoryFromUrl(url, name) };
  } catch (error) {
...
async function scrapeASOSProduct(url: string) {
  try {
    ...
    const imageMatch = html.match(/<meta property="og:image" content="([^"]+)"/i);
    const image = imageMatch ? imageMatch[1] : '';
    return { name, price, image, brand: 'ASOS', category: detectCategoryFromUrl(url, name) };
  } catch (error) {
...
async function scrapeCOSProduct(url: string) {
  try {
    ...
    const imageMatch = html.match(/<meta property="og:image" content="([^"]+)"/i);
    const image = imageMatch ? imageMatch[1] : '';
    return { name, price, image, brand: 'COS', category: detectCategoryFromUrl(url, name) };
  } catch (error) {
...
async function scrapeAmazonProduct(url: string) {
  try {
    ...
    return { name, price, image, brand: extractBrandFromUrl(url), category: detectCategoryFromUrl(url, name) };
  } catch (error) {
...
async function scrapeMacyProduct(url: string) {
  try {
    ...
    const imageMatch = html.match(/<meta property="og:image" content="([^"]+)"/i);
    const image = imageMatch ? imageMatch[1] : '';
    return { name, price, image, brand: 'Macy\'s', category: detectCategoryFromUrl(url, name) };
  } catch (error) {
...
async function scrapeGapProduct(url: string) {
  try {
    ...
    return { name, price, image, brand, category: detectCategoryFromUrl(url, name), description: productData?.description || '' };
  } catch (error) {
...
async function scrapeGenericProduct(url: string) {
  try {
    ...
    return { name, price, currency, image, description, brand: extractBrandFromUrl(url), category: detectCategoryFromUrl(url, name) };
  } catch (error) {
...
function extractBrandFromUrl(url: string): string {
  try {
    ...
    let brand = mainDomain.replace(/^www\./, '').replace(/^shop\./, '').replace(/^store\./, '')
